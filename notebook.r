...
mutate(next_chord=lead(chord,1),next_title=lead(title,1),bigram=paste(chord," ",next_chord))%>%
...
bb_tagged %>%
  ...
  ggplot(aes(x=chord, y=n, fill=theme(legend.position='none'))) +
  geom_bar() +
